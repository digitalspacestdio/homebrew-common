...
for FORMULA in $FORMULAS; do
    ...
    for jsonfile in ./*.json; do
        ...
        if ! [[ -z $JSON_FORMULA_NAME ]]; then
            mergedfile=$(jq -r '.["digitalspacestdio/common/'$JSON_FORMULA_NAME'"].formula.name + "-" + ."digitalspacestdio/common/'$JSON_FORMULA_NAME'".formula.pkg_version + ".json"' "$jsonfile" | perl -pe 's/\+/\ /g;' -e 's/%(..)/chr(hex($1))/eg;')
            set -x
            ...
            done < <(jq -r '."digitalspacestdio/common/'$JSON_FORMULA_NAME'".bottle.tags[].filename' "$jsonfile" | perl -pe 's/\+/\ /g;' -e 's/%(..)/chr(hex($1))/eg;')
            set +x
